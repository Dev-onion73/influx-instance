...
def _compute_site_summary_l2(
    ...
    if RAW_NODE_ID_COL not in scored_df.columns:
        site_name = "single_site"
        rs_now = float(scored_df["node_risk"].mean())
        site_risk_history.setdefault(site_name, []).append(rs_now)
        site_risk_history[site_name] = site_risk_history[site_name][-trend_window_size:]
        hist = site_risk_history[site_name]
        ...
        return pd.DataFrame(
            {
                ...
                "nodes": [int(len(scored_df))],
            }
        )

    grouped = scored_df.groupby(RAW_NODE_ID_COL, dropna=False)
    site = grouped["node_risk"].agg(["mean", "max", "count"]).rename(
        columns={"mean": "Rs_now", "max": "Rs_peak", "count": "nodes"}
    )

    # Update and retain per-site history across polling cycles.
    for site_name, rs_now in site["Rs_now"].items():
        key = str(site_name)
        site_risk_history.setdefault(key, []).append(float(rs_now))
        site_risk_history[key] = site_risk_history[key][-trend_window_size:]

    site["Rs_prev"] = site.index.map(
        lambda s: (
            float(sum(site_risk_history[str(s)][:-1]) / max(1, len(site_risk_history[str(s)]) - 1))
            if len(site_risk_history.get(str(s), [])) > 1
            else float(site.loc[s, "Rs_now"])
        )
    )
    site["Rs_smoothed"] = alpha * site["Rs_now"] + (1.0 - alpha) * site["Rs_prev"]
    site["dRs_dt"] = site.index.map(
        lambda s: (
            float((site_risk_history[str(s)][-1] - site_risk_history[str(s)][0]) / max(1, len(site_risk_history[str(s)]) - 1))
            if len(site_risk_history.get(str(s), [])) > 1
            else 0.0
        )
    )

    dist = grouped["dominant_issue"].value_counts(normalize=True).rename("share").reset_index()
    pivot = dist.pivot(index=RAW_NODE_ID_COL, columns="dominant_issue", values="share").fillna(0.0)
    site = site.join(pivot, how="left").reset_index().rename(columns={RAW_NODE_ID_COL: "site"})
    return site
